...
ImFont* winmain::FontBig = nullptr;
...
unsigned winmain::updateCounter = 0, winmain::frameCounter = 0;
double winmain::UpdateToFrameCounter = 0;
winmain::DurationMs winmain::sleepRemainder{0}, winmain::frameDuration{};
winmain::TimePoint winmain::frameStart, winmain::prevTime;
...
#ifdef __EMSCRIPTEN__
		// 웹: 언어 선택은 Restart()에서 localStorage 에 저장 후 페이지를 리로드한다. .ini(MEMFS)는
		// 새로고침 시 초기화되므로, 저장된 언어를 폰트 아틀라스 빌드(아래) 전에 복원해야
		// GetGlyphRange 가 해당 언어의 글리프(예: 한글)를 아틀라스에 포함시킨다.
		{
			char langBuf[16] = {0};
			int haveLang = EM_ASM_INT({
				try {
					var l = localStorage.getItem('scp_lang') || '';
					if (!l) return 0;
					stringToUTF8(l, $0, $1);
					return 1;
				} catch (e) { return 0; }
			}, langBuf, (int)sizeof(langBuf));
			if (haveLang)
			{
				Options.Language.V = langBuf;
				translations::SetCurrentLanguage(langBuf);
			}
		}
		// 웹: 번들된 한글 폰트(네오둥근모 서브셋)를 기본 폰트로 사용(사용자 미지정 시).
		// webdata/fonts/korean.ttf → preload 로 /fonts/korean.ttf 에 올라온다.
		if (Options.FontFileName.V.empty())
			Options.FontFileName.V = "/fonts/korean.ttf";
#endif
...
#ifdef __EMSCRIPTEN__
			// 픽셀 폰트(네오둥근모): 오버샘플 끄고 픽셀 스냅 → 각진 도트가 또렷하게.
...
#ifdef __EMSCRIPTEN__
				// 메뉴용 기본 폰트(16px) + 인게임 텍스트박스용 큰 폰트(32px). 32px 는 TTextBox 에서 PushFont.
				if (io.Fonts->AddFontFromFileTTF(fileName, 16.f, &fontConfig, ranges.Data))
					fontLoaded = true;
				FontBig = io.Fonts->AddFontFromFileTTF(fileName, 32.f, &fontConfig, ranges.Data);
#else
...
#ifdef __EMSCRIPTEN__
	// Browsers own the frame clock: register one tick per animation frame and yield control.
	// simulate_infinite_loop = 1 unwinds the C++ stack here; the loop keeps running async.
	emscripten_set_main_loop(&winmain::MainLoopTick, 0, 1);
#else
	while (!bQuit)
		MainLoopTick();
...
{
	{
		...
		if (has_focus)
		{
			...
			TimePoint frameEnd;
#ifdef __EMSCRIPTEN__
			// No blocking sleep in the browser; requestAnimationFrame already paces frames.
			frameEnd = updateEnd;
#else
...
{
#ifdef __EMSCRIPTEN__
	// 웹: do{...}while(restart) 재초기화가 도달 불가(MainLoop 이 emscripten_set_main_loop 로
	// 스택을 언와인드하고 리턴하지 않음). 현재 언어를 저장하고 페이지를 리로드해 정상 초기화
	// 경로로 재구성한다. scp_autostart 로 셸이 닉/방 입력을 건너뛰고 경쟁 세션을 자동 재개한다.
	const char* shortName = translations::GetCurrentLanguage()->ShortName;
	EM_ASM({
		try {
			localStorage.setItem('scp_lang', UTF8ToString($0));
			localStorage.setItem('scp_autostart', '1');
		} catch (e) {}
		location.reload();
	}, shortName);
#else
...
// ── Web(WASM) 점수 브릿지 ─────────────────────────────────────────
// JS(shell.html)가 ccall 로 폴링해 현재 점수/게임오버를 읽어 서버로 전송한다.
// 모두 읽기 전용. MainTable 은 게임 리셋 전 nullptr 일 수 있어 가드한다.
#ifdef __EMSCRIPTEN__
#include <emscripten.h>
...
extern "C"
{
	// 현재 총점. double 반환 — ccall 'number' 는 i64 를 못 읽고, 점수는 2^53 안이라 double 로 정확.
	EMSCRIPTEN_KEEPALIVE double scp_score()
	{
		auto t = pb::MainTable;
		if (!t)
			return -1;
		return static_cast<double>(t->CurScoreE9) * 1e9 + t->CurScore;
	}

	// 게임 오버 여부 (1/0). 매 프레임 갱신되는 pb::game_mode 기준.
	EMSCRIPTEN_KEEPALIVE int scp_is_gameover()
	{
		return pb::game_mode == GameModes::GameOver ? 1 : 0;
	}

	// 남은 볼 개수 (없으면 -1).
	EMSCRIPTEN_KEEPALIVE int scp_ball_count()
	{
		auto t = pb::MainTable;
		return t ? t->BallCount : -1;
	}
}
#endif
