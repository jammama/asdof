...
- HUD: 세이브(💾, 상태저장 슬롯 6개) · 빨리감기(≫) · 가로세로 전환(⟳) · 설정(⚙) · 전체화면(⛶) · 목록으로.
- 배터리 세이브(게임 내 "저장/리포트")는 자동으로 IndexedDB에 영속화된다
  (플레이 중 주기적 + 뒤로가기/탭 숨김 시). 마지막 지점 자동 이어하기도 지원.

## 서버 세이브 (기기 간 이어하기)

세이브를 서버에 올려두고 다른 기기에서 받아 이어할 수 있다. 저장소는 **별도 서비스
[`asdof-saves`](../asdof-saves)** (범용 세이브 스토리지)를 쓴다 — 에뮬레이터엔 백엔드가 없다(디커플).

- 라이브러리의 **☁ 저장 파일** → 이 기기 세이브를 서버에 올리거나 / 서버 세이브를 받기.
- 설정 ⚙ > 서버 에 **서버 주소**(기본 `https://saves.asdof.xyz`)와 **토큰**을 입력.
- `asdof-saves` 를 먼저 배포해야 동작한다 (해당 프로젝트 README 참고). 네임스페이스는 `asdof-emulator`.
...
js/
  ...
  library.js          롬 목록/업로드/삭제 + 서버 선반 + 로컬 세이브 파일
  server-saves.js     asdof-saves(외부 서비스) 세이브 업/다운로드
  player.js           실행/상태저장(슬롯)/빨리감기/일시정지
