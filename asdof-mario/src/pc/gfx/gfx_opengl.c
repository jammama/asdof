...
static struct ShaderProgram *gfx_opengl_create_and_load_new_shader(uint32_t shader_id) {
    ...
    append_line(vs_buf, &vs_len, "#version 100");   // WebGL: GLSL ES 1.00 (데스크톱 110 은 WebGL 에서 컴파일 실패)
#else
    ...
    // Fragment shader
#ifdef __EMSCRIPTEN__
    append_line(fs_buf, &fs_len, "#version 100");            // WebGL: GLSL ES 1.00
    append_line(fs_buf, &fs_len, "precision mediump float;"); // ES 는 프래그먼트 float 정밀도 지정 필수
#else
