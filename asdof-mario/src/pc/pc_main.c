...
static void on_anim_frame(double time) {
    ...
    // 게임 초기화(gfx/오디오/첫 레벨 로딩)가 끝나기 전에는 프레임을 생성하지 않는다.
    // ASYNCIFY 때문에 main_func 의 초기화 도중 yield 가 발생하면, 그 틈에 rAF 가 먼저
    // 발화해 미초기화 상태에서 game_loop → 레벨 로더가 돌며 main_pool_alloc 의 malloc 이
    // abort 된다(= "native code called abort()"). inited 될 때까지 그냥 재예약만 한다.
    if (!inited) {
